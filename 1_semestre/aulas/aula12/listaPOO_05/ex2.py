...
class BoletoUI:
    ...
    @classmethod
    def atualizar(cls):
        cod = int(input('Código do Boleto: '))
    ...
    @classmethod
    def excluir(cls):
        cod = int(input('Código do boleto a ser excluído: '))
        for x in cls.__boletos:
            if x.get_cod() == cod:
                cls.__boletos.remove(x)

    @classmethod
    # mostra os que ainda não foram pagos (todo, inclussive os que já venceram)
    def em_aberto(cls):
        for x in cls.__boletos:
    ...
    @classmethod
    # mostra os que não foram pagos e ainda não veceram
    def a_vencer(cls):
        for x in cls.__boletos:
    ...
    @classmethod
    def pagar(cls):
        print('Boletos não pagos: ')
        for x in cls.__boletos:
            if x.get_situacao_boleto() == Pagamento.EM_ABERTO:
                print(x)
        
        cod = int(input('Código do Boleto: '))
        for x in cls.__boletos:
            if x.get_cod() == cod:
                valor = float(input('O valor que você vai pagar: '))
                x.pagar(valor)
                print(f'{x.get_situacao_pagamento()}')
